...
var promisifyStream = (stream) => new Proxy(stream, {
  get(target, key) {
    if (key === "then") {
      return (res = noop, rej = noop) => new Promise(
        (_res, _rej) => target.once("error", (e) => _rej(rej(e))).once("finish", () => _res(res(target))).once("end-piped-from", () => _res(res(target)))
      );
    }
